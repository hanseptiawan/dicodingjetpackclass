package dev.craftid.dicodingjetpack.utils;

import android.util.Log;

import java.util.ArrayList;
import java.util.List;

import dev.craftid.dicodingjetpack.data.ContentEntity;

public class DataDummy {

    public static List<ContentEntity> getMovieDummies(){
        List<ContentEntity> contentEntities = new ArrayList<>();

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                0,
                "7.5",
                "A Star Is Born",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/wrFpXMNBRj2PBiN4Z5kix51XaIZ.jpg",
                "A Star Is Born",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Seasoned musician Jackson Maine discovers and falls in love with struggling artist Ally. She has just about given up on her dream to make it big as a singer until Jack coaxes her into the spotlight. But even as Allys career takes off, the personal side of their relationship is breaking down, as Jack fights an ongoing battle with his own internal demons.",
                "October 5, 2018",
                null
                ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                1,
                "6.8",
                "Aquaman",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/5Kg76ldv7VxeX9YlcQXiowHgdX6.jpg",
                "Aquaman",
                "https://img.youtube.com/vi/WDkg3h8PCVU/hqdefault.jpg",
                "Once home to the most advanced civilization on Earth, Atlantis is now an underwater kingdom ruled by the powerhungry King Orm. With a vast army at his disposal, Orm plans to conquer the remaining oceanic people and then the surface world. Standing in his way is Arthur Curry, Orms halfhuman, halfAtlantean brother and true heir to the throne.",
                "December 21, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                2,
                "8.3",
                "Avengers : Infinity War",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/7WsyChQLEftFiDOVTGkv3hFpyyt.jpg",
                "Avengers : Infinity War",
                "https://img.youtube.com/vi/6ZfuNTqbHE8/hqdefault.jpg",
                "As the Avengers and their allies have continued to protect the world from threats too large for any one hero to handle, a new danger has emerged from the cosmic shadows: Thanos. A despot of intergalactic infamy, his goal is to collect all six Infinity Stones, artifacts of unimaginable power, and use them to inflict his twisted will on all of reality. Everything the Avengers have fought for has led up to this moment  the fate of Earth and existence itself has never been more uncertain.",
                "April 23, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                3,
                "7.0",
                "Bird Box",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/rGfGfgL2pEPCfhIvqHXieXFn7gp.jpg",
                "Bird Box",
                "https://img.youtube.com/vi/MPWMBITCudM/hqdefault.jpg",
                "Five years after an ominous unseen presence drives most of society to suicide, a survivor and her two children make a desperate bid to reach safety.",
                "November 12, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                4,
                "8.1",
                "Bohemian Rhapsody",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/lHu1wtNaczFPGFDTrjCSzeLPTKN.jpg",
                "Bohemian Rhapsody",
                "https://img.youtube.com/vi/mP0VHJYFOAU/hqdefault.jpg",
                "Singer Freddie Mercury, guitarist Brian May, drummer Roger Taylor and bass guitarist John Deacon take the music world by storm when they form the rock n roll band Queen in 1970. Hit songs become instant classics. When Mercurys increasingly wild lifestyle starts to spiral out of control, Queen soon faces its greatest challenge yet – finding a way to keep the band together amid the success and excess.",
                "October 30, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                5,
                "6.5",
                "Bumblebee",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/fw02ONlDhrYjTSZV8XO6hhU3ds3.jpg",
                "Bumblebee",
                "https://img.youtube.com/vi/lcwmDAYt22k/hqdefault.jpg",
                "On the run in the year 1987, Bumblebee finds refuge in a junkyard in a small Californian beach town. Charlie, on the cusp of turning 18 and trying to find her place in the world, discovers Bumblebee, battlescarred and broken. When Charlie revives him, she quickly learns this is no ordinary yellow VW bug.",
                "December 21, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                6,
                "6.7",
                "Creed II",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/v3QyboWRoA4O9RbcsqH8tJMe8EB.jpg",
                "Creed II",
                "https://img.youtube.com/vi/ApQbQ0iJQO0/hqdefault.jpg",
                "Between personal obligations and training for his next big fight against an opponent with ties to his familys past, Adonis Creed is up against the challenge of his life.",
                "November 14, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                7,
                "6.9",
                "Once Upon a Deadpool",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/5Ka49BWWyKMXr93YMbH5wLN7aAM.jpg",
                "Once Upon a Deadpool",
                "https://img.youtube.com/vi/PCf03KXyzIg/hqdefault.jpg",
                "A kidnapped Fred Savage is forced to endure Deadpools PG13 rendition of Deadpool 2 as a Princess Brideesque story thats full of magic, wonder &amp; zero Fs.",
                "December 11, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                8,
                "7.6",
                "How to Train Your Dragon : The Hidden World",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/xvx4Yhf0DVH8G4LzNISpMfFBDy2.jpg",
                "How to Train Your Dragon : The Hidden World",
                "https://img.youtube.com/vi/SkcucKDrbOI/hqdefault.jpg",
                "As Hiccup fulfills his dream of creating a peaceful dragon utopia, Toothless’ discovery of an untamed, elusive mate draws the Night Fury away. When danger mounts at home and Hiccup’s reign as village chief is tested, both dragon and rider must make impossible decisions to save their kind.",
                "February 22, 2019",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                9,
                "7.4",
                "Dragon Ball Super : Broly",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/f03YksE4NggUjG75toz4H1YAGRf.jpg",
                "Dragon Ball Super : Broly",
                "https://img.youtube.com/vi/FHgm89hKpXU/hqdefault.jpg",
                "Earth is peaceful following the Tournament of Power. Realizing that the universes still hold many more strong people yet to see, Goku spends all his days training to reach even greater heights. Then one day, Goku and Vegeta are faced by a Saiyan called Broly who theyve never seen before. The Saiyans were supposed to have been almost completely wiped out in the destruction of Planet Vegeta, so whats this one doing on Earth? This encounter between the three Saiyans who have followed completely different destinies turns into a stupendous battle, with even Frieza (back from Hell) getting caught up in the mix.",
                "December 13, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                10,
                "6.5",
                "Glass",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/svIDTNUoajS8dLEo7EosxvyAsgJ.jpg",
                "Glass",
                "https://img.youtube.com/vi/95ghQs5AmNk/hqdefault.jpg",
                "In a series of escalating encounters, former security guard David Dunn uses his supernatural abilities to track Kevin Wendell Crumb, a disturbed man who has twentyfour personalities. Meanwhile, the shadowy presence of Elijah Price emerges as an orchestrator who holds secrets critical to both men.",
                "January 18, 2019",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                11,
                "6.3",
                "Hunter Killer",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/a0j18XNVhP4RcW3wXwsqT0kVoQm.jpg",
                "Hunter Killer",
                "https://img.youtube.com/vi/mnP_z3qXDCQ/hqdefault.jpg",
                "Captain Glass of the USS Arkansas discovers that a coup detat is taking place in Russia, so he and his crew join an elite group working on the ground to prevent a war.",
                "October 26, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                12,
                "6.6",
                "Mary Poppins Returns",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/uTVGku4LibMGyKgQvjBtv3OYfAX.jpg",
                "Mary Poppins Returns",
                "https://img.youtube.com/vi/3jsfXDZLIY/hqdefault.jpg",
                "In Depressionera London, a nowgrown Jane and Michael Banks, along with Michaels three children, are visited by the enigmatic Mary Poppins following a personal loss. Through her unique magical skills, and with the aid of her friend Jack, she helps the family rediscover the joy and wonder missing in their lives.",
                "November 29, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                13,
                "6.0",
                "Mortal Engines",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/iteUvQKCW0EqNQrIVzZGJntYq9s.jpg",
                "Mortal Engines",
                "https://img.youtube.com/vi/IRsFc2gguEg/hqdefault.jpg",
                "Many thousands of years in the future, Earth’s cities roam the globe on huge wheels, devouring each other in a struggle for ever diminishing resources. On one of these massive traction cities, the old London, Tom Natsworthy has an unexpected encounter with a mysterious young woman from the wastelands who will change the course of his life forever.",
                "December 14, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                14,
                "6.6",
                "Preman Pensiun",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/7xfWyoz4SF5LHZ713eMtC2aZ0lT.jpg",
                "Preman Pensiun",
                "https://img.youtube.com/vi/WnB7YMFsKFI/hqdefault.jpg",
                "Bahars Full Inspiration Comedy Series is actually just a \"small\" thug, but the area is quite extensive, besides being a \"backing\" of street vendors, it also controls a market and terminal. The story that will be told in this series is not the journey of his life from the beginning, although in some dialogues he also tells, but the story in his old age when he decided to retire.",
                "January 17, 2019",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                15,
                "5.8",
                "Robin Hood",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/AiRfixFcfTkNbn2A73qVJPlpkUo.jpg",
                "Robin Hood",
                "https://img.youtube.com/vi/zwPn9ZnbCo0/hqdefault.jpg",
                "A warhardened Crusader and his Moorish commander mount an audacious revolt against the corrupt English crown.",
                "November 21, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                16,
                "8.4",
                "SpiderMan: Into the SpiderVerse",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/iiZZdoQBEYBv6id8su7ImL0oCbD.jpg",
                "SpiderMan: Into the SpiderVerse",
                "https://img.youtube.com/vi/tg52up16eq0/hqdefault.jpg",
                "Miles Morales is juggling his life between being a high school student and being a spiderman. When Wilson \"Kingpin\" Fisk uses a super collider, others from across the SpiderVerse are transported to this dimension.",
                "December 1, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                17,
                "6.0",
                "The Girl in The Spiders Web",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/37ebJLy8WbZ1VbOndkk1i4vtwdf.jpg",
                "The Girl in The Spiders Web",
                "https://img.youtube.com/vi/XKMSP9OKspQ/hqdefault.jpg",
                "In Stockholm, Sweden, hacker Lisbeth Salander is hired by Frans Balder, a computer engineer, to retrieve a program that he believes it is too dangerous to exist.",
                "November 9, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                18,
                "6.5",
                "The Mule",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/klazQbxk3yfuZ8JcfO9jdKOZQJ7.jpg",
                "The Mule",
                "https://img.youtube.com/vi/N_QksSzK7sI/hqdefault.jpg",
                "Earl Stone, a man in his 80s, is broke, alone, and facing foreclosure of his business when he is offered a job that simply requires him to drive. Easy enough, but, unbeknownst to Earl, he’s just signed on as a drug courier for a Mexican cartel. He does so well that his cargo increases exponentially, and Earl hit the radar of hardcharging DEA agent Colin Bates.",
                "December 10, 2018",
                null
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_MOVIE,
                19,
                "6.6",
                "Venom",
                null,
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/2uNW4WbgBXL25BAbXGLnLqX71Sw.jpg",
                "Venom",
                "https://img.youtube.com/vi/xLCn88bfW1o/hqdefault.jpg",
                "Investigative journalist Eddie Brock attempts a comeback following a scandal, but accidentally becomes the host of Venom, a violent, super powerful alien symbiote. Soon, he must rely on his newfound powers to protect the world from a shadowy organization looking for a symbiote of their own.",
                "October 1, 2018",
                null
        ));

        return contentEntities;
    }

    public static List<ContentEntity> getTVShowDummies(){
        List<ContentEntity> contentEntities = new ArrayList<>();

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                20,
                "5.8",
                "Arrow",
                "Arrow",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/mo0FP1GxOFZT4UDde7RFDz5APXF.jpg",
                "Arrow",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Spoiled billionaire playboy Oliver Queen is missing and presumed dead when his yacht is lost at sea. He returns five years later a changed man, determined to clean up the city as a hooded vigilante armed with a bow.",
                null,
                "October 10, 2012"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                21,
                "8.1",
                "Game of Thrones",
                "Game of Thrones",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/u3bZgnGQ9T01sWNhyveQz0wH0Hl.jpg",
                "Game of Thrones",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Seven noble families fight for control of the mythical land of Westeros. Friction between the houses leads to full scale war. All while a very ancient evil awakens in the farthest north. Amidst the war, a neglected military order of misfits, the Nights Watch, is all that stands between the realms of men and icy horrors beyond.",
                null,
                "April 17, 2011"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                22,
                "6.4",
                "Hanna",
                "Hanna",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/5nSSkcM3TgpllZ7yTyBOQEgAX36.jpg",
                "Hanna",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "This thriller and coming of age drama follows the journey of an extraordinary young girl as she evades the relentless pursuit of an off book CIA agent and tries to unearth the truth behind who she is. Based on the 2011 Joe Wright film.",
                null,
                "March 28, 2019"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                23,
                "7.3",
                "The Walking Dead",
                "The Walking Dead",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/bjU4tLlyp8W4yTB0Hqn8J1IDUnD.jpg",
                "The Walking Dead",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Sheriffs deputy Rick Grimes awakens from a coma to find a post apocalyptic world dominated by flesh eating zombies. He sets out to find his family and encounters many other survivors along the way.",
                null,
                "October 31, 2010"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                24,
                "5.8",
                "Supergirl",
                "Supergirl",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/vqBsgL9nd2v04ZvCqPzwtckDdFD.jpg",
                "Supergirl",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Twenty four year old Kara Zor El, who was taken in by the Danvers family when she was 13 after being sent away from Krypton, must learn to embrace her powers after previously hiding them. The Danvers teach her to be careful with her powers, until she has to reveal them during an unexpected disaster, setting her on her journey of heroism.",
                null,
                "October 26, 2015"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                25,
                "6.9",
                "Riverdale",
                "Riverdale",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/gskv297rlbyzLaTU1XZf8UBbxp0.jpg",
                "Riverdale",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Set in the present, the series offers a bold, subversive take on Archie, Betty, Veronica and their friends, exploring the surreality of small town life, the darkness and weirdness bubbling beneath Riverdale’s wholesome facade.",
                null,
                "January 26, 2017"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                26,
                "7.3",
                "Supernatural",
                "Supernatural",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/3iFm6Kz7iYoFaEcj4fLyZHAmTQA.jpg",
                "Supernatural",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "When they were boys, Sam and Dean Winchester lost their mother to a mysterious and demonic supernatural force. Subsequently, their father raised them to be soldiers. He taught them about the paranormal evil that lives in the dark corners and on the back roads of America and he taught them how to kill it. Now, the Winchester brothers crisscross the country in their 67 Chevy Impala, battling every kind of supernatural threat they encounter along the way.",
                null,
                "September 13, 2005"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                27,
                "6.7",
                "The Flash",
                "The Flash",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/fki3kBlwJzFp8QohL43g9ReV455.jpg",
                "The Flash",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "After a particle accelerator causes a freak storm, CSI Investigator Barry Allen is struck by lightning and falls into a coma. Months later he awakens with the power of super speed, granting him the ability to move through Central City like an unseen guardian angel. Though initially excited by his newfound powers, Barry is shocked to discover he is not the only \"meta human\" who was created in the wake of the accelerator explosion and not everyone is using their new powers for good. Barry partners with S.T.A.R. Labs and dedicates his life to protect the innocent. For now, only a few close friends and associates know that Barry is literally the fastest man alive, but it wont be long before the world learns what Barry Allen has become The Flash.",
                null,
                "October 7, 2014"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                28,
                "6.8",
                "Gotham",
                "Gotham",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/4XddcRDtnNjYmLRMYpbrhFxsbuq.jpg",
                "Gotham",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Before there was Batman, there was GOTHAM. Everyone knows the name Commissioner Gordon. He is one of the crime worlds greatest foes, a man whose reputation is synonymous with law and order. But what is known of Gordons story and his rise from rookie detective to Police Commissioner? What did it take to navigate the multiple layers of corruption that secretly ruled Gotham City, the spawning ground of the worlds most iconic villains? And what circumstances created them the larger than life personas who would become Catwoman, The Penguin, The Riddler, Two Face and The Joker",
                null,
                "September 22, 2014"
        ));

        contentEntities.add(new ContentEntity(
                ContentEntity.TYPE_TV_SHOW,
                29,
                "7.8",
                "Shameless",
                "Shameless",
                "https://image.tmdb.org/t/p/w600_and_h900_bestv2/iRyQTp2L437k6zfFCvZSOWaxQFA.jpg",
                "Shameless",
                "https://img.youtube.com/vi/nSbzyEJ8X9E/hqdefault.jpg",
                "Chicagoan Frank Gallagher is the proud single dad of six smart, industrious, independent kids, who without him would be, perhaps better off. When Franks not at the bar spending what little money they have, hes passed out on the floor. But the kids have found ways to grow up in spite of him. They may not be like any family you know, but they make no apologies for being exactly who they are.",
                null,
                "January 9, 2011"
        ));

        return contentEntities;
    }

    public static ContentEntity getDetailContent(int contentID){
        ContentEntity newContentEntity = new ContentEntity();
        List<ContentEntity> contentEntities = new ArrayList<>();
        contentEntities.addAll(getMovieDummies());
        contentEntities.addAll(getTVShowDummies());

        for (ContentEntity entity : contentEntities){
            if (entity.getIdContent() == contentID){
                newContentEntity = entity;
            }
        }

        return newContentEntity;
    }

    public static List<ContentEntity> searchContent(String query) {
        List<ContentEntity> searchResults = new ArrayList<>();
        List<ContentEntity> contentEntities = new ArrayList<>();
        contentEntities.addAll(getMovieDummies());
        contentEntities.addAll(getTVShowDummies());

        for (ContentEntity contentEntity : contentEntities) {
            if (contentEntity.getTitle().toLowerCase().contains(query.toLowerCase())) {
                searchResults.add(contentEntity);
            }
        }
        return searchResults;
    }

}
